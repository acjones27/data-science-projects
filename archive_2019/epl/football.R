library (dplyr)
source ('data-science-projects/epl/clean_data.R')
...
# Simulate match
matches <- mapply(get_score, "Liverpool", "Everton", nsim, SIMPLIFY = FALSE)
score_line <- sapply(matches, function(x) x[1])
prob <- sapply(matches, function(x) x[2])
