...
fixtures <- read.csv("https://raw.githubusercontent.com/tuangauss/DataScienceProjects/master/data/fixtures.csv", stringsAsFactors=FALSE)

# get the team
teams <- unique(fixtures$HOME.TEAM)

# extract historic results
history <- read.csv("https://raw.githubusercontent.com/tuangauss/DataScienceProjects/master/data/history.csv", stringsAsFactors=FALSE)

# get info from the 2010 up to 2018
seasons <- sapply(10: 17, function(x)
  paste0(2000 + x, '-', x + 1))
